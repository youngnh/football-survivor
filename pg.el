...
(defstruct pgcon process pid secret (binaryp nil))
...
\"localhost\", and PASSWORD to an empty string."  
  (let* ((buf (generate-new-buffer " *PostgreSQL*"))
    ...
    (setq connection (make-pgcon :process process))
...
(defun pg:read-char (connection)
  ...
  (let ((process (pgcon-process connection)))
    (with-current-buffer (process-buffer process)
      (unless (char-after 1)
        (accept-process-output process 0 1000)
	(while (not (char-after 1))
	  (accept-process-output process 0 100000)))
      (prog1 (char-after 1)
  ...
  (let* ((process (pgcon-process connection)))
    (with-current-buffer (process-buffer process)
      (when (< (buffer-size) howmany)
        (accept-process-output process 0 2000)
	(while (< (buffer-size) howmany)
	  (accept-process-output process 0 200000)))
